...
const rest = new discord_js_1.REST().setToken(config_1.config.CLOCKEY_TOKEN);
async function deployCommands({ guildId }) {
    try {
        console.log("Started refreshing application (/) commands.");
        await rest.put(discord_js_1.Routes.applicationGuildCommands(config_1.config.CLOCKEY_ID, guildId), {
            body: commandsData,
        });
